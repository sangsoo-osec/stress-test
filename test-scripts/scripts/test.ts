...
export const main = async () => {
    ...
    let orders_in_tree = 0;
    for(let i = 0; i < 5; i++) {
        await place_limit_order(signer, suiiType, usdcType, deepbookPackageId, pool_sui_usdc, balance_manager, 100 * FLOAT_SCALING, MIN_SIZE, true, 100, false);
        orders_in_tree += 100;
        console.log("#orders_in_tree", orders_in_tree);
    }

    console.log("end place limit order");

    console.log("try filling 300 ordees within single tx");
    await place_limit_order(signer, suiiType, usdcType, deepbookPackageId, pool_sui_usdc, balance_manager2, 100 * FLOAT_SCALING, 300 * MIN_SIZE, false, 1, true);
    orders_in_tree -= 300;
    console.log("#orders_in_tree", orders_in_tree);
    ...
    console.log("try filling 300 orders within single tx");
    await place_limit_order(signer, suiiType, usdcType, deepbookPackageId, pool_sui_usdc, balance_manager2, 100 * FLOAT_SCALING, 350 * MIN_SIZE, false, 1, true);
    orders_in_tree -= 300;
    console.log("#orders_in_tree", orders_in_tree);
